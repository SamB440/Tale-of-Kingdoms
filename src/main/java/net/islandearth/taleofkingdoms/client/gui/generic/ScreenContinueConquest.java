...
public class ScreenContinueConquest extends ScreenTOK {
	...
	public void render(MatrixStack stack, int par1, int par2, float par3) {
		...
		this.drawCenteredString(stack, this.textRenderer, MinecraftClient.getInstance().player.getName().getString()
				+ ", your conquest, " 
				+ instance.getName() + ", has come far.", this.width / 2, this.height / 2 + 40, 0xFFFFFF);
		this.drawCenteredString(stack, this.textRenderer, "Now you seek to venture further, and continue your journey.", this.width / 2, this.height / 2 + 50, 0xFFFFFF);
		this.drawCenteredString(stack, this.textRenderer, "Safe travels, and go forth!", this.width / 2, this.height / 2 + 60, 0xFFFFFF);
